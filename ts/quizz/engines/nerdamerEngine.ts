...
import { EngineCAS, PayloadCAS, PayloadRulesCAS, ResponseCAS } from "./engineCAS";
import { parseLatexNerdamer } from "./parseLatexNerdamer"
...
class NerdamerCAS implements EngineCAS {
    ...
    private checkRules(p: any): string[] {
        ...
        if (rules['forbidden']) {
           ...
           rules["forbidden"].forEach( (fo_symb:string) => {
                if(latex_code[fo_symb]) {
                    rules_checks.push(getI18n(this.lang, 'sym_notallowed', fo_symb))
                }
           })
        }
        if (rules["unique"]) {
            ...
            rules["unique"].forEach( (un_sym:string) => {
                if((latex_code.match(new RegExp(un_sym, 'g')) || []).length > 1) {
                    rules_checks.push(getI18n(this.lang, 'sym_once', un_sym))
                }
            })
        }
    ...
    compare(p: PayloadCAS): Promise<ResponseCAS> {
        ...
        return new Promise<any>((resolve, reject) => {
            ...
            const rules_checks =this.checkRules(p)
            ...
            let user_nerd = []
            try {
                user_nerd = p['latex'].map( (e: string) => parseLatexNerdamer(e))
                if(user_nerd.filter((x: any) => x==null).length > 0) {
                    return resolve({qid: p.qid, correct: -4, msg: getI18n(this.lang, 'error_cantprocess')})
                }
            } catch(ex) {
                console.error(ex)
            ...
            user_nerd.forEach( (v: any, i: number) => {
                cas.setVar('ua_'+i, v)
            })
            ...
            try {
                ...
                ans_nerd = p['ans'].map( (e: string) => cas(e, scope) )
                if ( ans_nerd.filter( (e: any) => e==null).length > 0) {
            ...
            try{ 
                ...
                zip(ans_nerd, user_nerd).forEach( (pair: any[]) => { 
            ...
            try {
                ...
                zip(ans_nerd, user_nerd).forEach( (pair: any[]) => {
                    ...
                    let expr = null
                    const rules = p["rules"] || {} as PayloadRulesCAS;
                    ...
                    if( typeof expr === 'boolean' && expr===true) {
                        ...
                        correct = 1
                        msg = getI18n(this.lang, 'right_answer')
                        /*
                        ...
                        */
                        ...
                        msg = getI18n(this.lang, 'wrong_answer')
                        correct = 0
                        /*
                        if is_matrix(expr):
                ...
                */
                ...
                msg = getI18n(this.lang, 'error_verifying')
                correct = -1 
            }
    ...
    getAnswer(p: any): Promise<any> {
        ...
        if(typeof p['ans'] === 'string') {
            const ans_nerd = cas(p['ans'], scope)
            const ra = `${getI18n(this.lang, 'expected_ans')}<br> \\(${ans_nerd.latex()}\\)`
...
let nerdamerCAS: NerdamerCAS;

export function getNerdamerCAS(lang: string): EngineCAS {
    if(!nerdamerCAS) {
